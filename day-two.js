const games = `Game 1: 9 red, 2 green, 13 blue; 10 blue, 2 green, 13 red; 8 blue, 3 red, 6 green; 5 green, 2 red, 1 blue
...
function parseGameValue(game) {
    let gameName = game.split(":")
    return gameName[0].split(" ")[1]
}


function getSumOfGames(games) {
    let sum = 0;
    let gamesArray = parseList(games)

    // for each game, evavluate if any of the dice numbers are 
    // larger than permissible 12 red, 13 green, 14 blue
        ...
        for (const grab of grabsArray) {
           let diceCheck = grab.split(", ")
            for (const dice of diceCheck) {
                ...
                let number = diceArray[0]
                let color = diceArray[1]
                if (number > colorValues[color]) {
                    passingGame = false
                }
            }
        }
...
function getSumOfCubesofCubes(games) {
    let sum = 0;
    let gamesArray = parseList(games)
    ...
    for (let i = 0; i < gamesArray.length; i++) {
        ...
        for (const grab of grabsArray) {
           let diceCheck = grab.split(", ")
            for (const dice of diceCheck) {
                let diceArray = dice.split(" ")
                let number = parseInt(diceArray[0])
                let color = diceArray[1]
                if (color === "blue") {
