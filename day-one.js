...
function numStringCheck(string, pos) {
    ...
    if (numbers[string.slice(pos, pos + 3)]) {
        ...
        return numbers[string.slice(pos, pos +4)]
    } else if (numbers[string.slice(pos, pos + 5)]) {
        return numbers[string.slice(pos, pos +5)]
    } else {
        return null
    }
}

function calibrationValuesPartTwo(list) {
    let sum = 0;
    let lines = list.split(/\r?\n/)
    for (const line of lines) {
        let first = null;
        ...
        for (let i = 0; i < line.length; i++) {
            ...
            } else if (numStringCheck(line, i)) {
                if (first === null) {
                    first = numStringCheck(line, i);
                    last = numStringCheck(line, i);
                } else {
                    last = numStringCheck(line, i);
                }
            }
...
console.log('part one', calibrationValues(calibrationList));
console.log('part two', calibrationValuesPartTwo(calibrationList));